...
const Form = ({ currentId, setCurrentId }) => {
  const [postData, setPostData] = useState({ creator: '', email: '', phone: '', address: '', selectedFile: '' });
  ...
  const clear = () => {
    setCurrentId(0);
    setPostData({ creator: '', email: '', phone: '', address: '',  selectedFile: '' });
  };
  ...
  return (
    <Paper className={classes.paper}>
      <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        <Typography variant="h6">{currentId ? `Editing "${post.creator}"` : 'Uploading'}</Typography>
